...
def preprocess(img, thresh, img_mult=255, hole_size=300,
    ...
    # if img.size < 0:
        ...
        img = remove_small_objects(img, max_size=hole_size)
        img = remove_small_holes(img, max_size=hole_size)
...
def img_to_ske_G(params):
    ...
    if verbose:
        print("Execute sknw...")
    # NOTE (mask_postprocessing): upstream picks sknw_int64 above 32767 px; that
    # variant's bare @jit no longer builds under numba 0.65, and our masks are
    # 1024 px, so the branch is dropped.
